...
export default class HelpContent {
  ...
  update() {
    ...
    if (router.page !== 'home') {
      this.unsubscribe();
      console.log('HelpContent unsubscribed');
      return;
    }

    if (this.state.help === home.help) return;

    this.setState({ help: home.help });
    this.container.classList.toggle('active', this.state.help);
    this.render();
  }

  handleClick({ target }) {
    if (!target.matches('.confirm-help') && !target.matches('.exit-help')) return;
    this.store.dispatch(toggleHelp());
  }

  render() {
    this.container.innerHTML = this.state.help
      ? `<article>
        ...
        <button class="confirm-help icon-check">OK</button>
        <button class="exit-help icon-cancel" aria-label="exit"></button>
      </article>`
      : '';
